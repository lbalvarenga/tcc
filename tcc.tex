...
\begin{resumo}
	...
	Este trabalho tem por objetivo verificar o impacto da inflação, principalmente sob a ótica da oferta e da demanda sobre a economia brasileira e avaliar os efeitos destas sobre a eficácia do regime de metas de inflação (RMI) no Brasil. Será examinado se as respostas do Banco Central são eficientes e se as óticas atuais da condução da política monetária atendem de forma eficiente os objetivos propostos pelo RMI. Por meio deste trabalho, busca-se atingir um melhor entendimento dos mecanismos que regem a dinâmica inflacionária no Brasil e como é moldada a resposta à inflação pela autoridade monetária brasileira. Para isso, será utilizado o modelo econométrico de vetores autorregressivos (VAR), que consegue capturar relações de longo prazo entre as variáveis escolhidas, que são a meta de inflação anual do Banco Central, a taxa de câmbio entre o Real e o Dólar Americano, o produto interno bruto (PIB) real do Brasil, o Índice Nacional de Preços ao Consumidor Amplo (IPCA) e o Índice de \textit{Commodities} - Brasil (IC-Br).
...
TODO: plotar SELIC e explicar essa dinamica. sobrecompensação etc.
...
Na equação \ref{eqn:varmodl} abaixo, é apresentado o modelo VAR de ordem $p$, onde um vetor de variáveis endógenas $y_t$ de dimensão $n \times 1$ é descrito por:
...
Para determinar corretamente como proceder com a diferenciação de uma série, surge a necessidade de verificar se cada série temporal possui raiz unitária e, portanto, não é estacionária. Nesse contexto, as seções subsequentes apresentam procedimentos para que isso seja determinado.
...
Levando o que foi apresentado em consideração, a seguir, é apresentada a Tabela \ref{table:vars}, que sintetiza as variáveis, suas origens e as siglas que foram adotadas durante a execução da rotina econométrica desenvolvida no \textit{software} STATA:
...
Os resultados dos testes DF-GLS e Phillips-Perron estão elencados na tabela \ref{table:adfpptests} abaixo. É evidente que as séries temporais não são estacionárias para nenhuma das variáveis a um nível de significância de $5\%$. Como observação, o prefixo "$\ln\_$" indica que a variável em questão foi transformada em logaritmo.
...
Observa-se que enquanto que o critério de informação de Schwarz indica o uso de uma defasagem, os outros critérios, AIC e HQIC apontam quatro defasagens como ideais para o modelo. Uma vez que o SBIC pune mais severamente subsequentes defasagens e que as séries temporais se extendem por um período de 20 anos, optamos por utilizar 4 \textit{lags}. Ao executar o modelo e obter sua IRF para as variáveis \textit{d.ln\_icbr} e \textit{d.ln\_pibinfl}, obtemos a figura \ref{fig:irf_ipca} abaixo:
...
\begin{figure}[H]
	...
	\includegraphics[width=\textwidth]{fig/irf_ipca.png}\\
	\footnotesize{\textbf{Fonte}: o próprio autor, dados fornecidos pelo Banco Central e Fundação Getúlio Vargas.}
	\label{fig:irf_ipca}
\end{figure}

TODO: Explicar que o lado de oferta não é bem suportado pelo RMI, como demonstra a IRF.
...
TODO: explicar que boa parte da variacao IPCA é formada por choques de oferta que nao sao bem resolvidas pela alteracao da SELIC.

TODO: executar o mesmo modelo para 2019-2024 (verificar impacto especifico da COVID)
TODO: demonstrar outros choques (efeito pass-through)
