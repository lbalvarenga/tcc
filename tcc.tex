...
% %%%%%%%%%%%%%%%%
%NOME DO TRABALHO 
\titulo{\bfseries ANÁLISE DA DINÂMICA INFLACIONÁRIA SOB O REGIME DE METAS DE INFLAÇÃO NO BRASIL ENTRE 2002 e 2024}
		...
		\begin{folhadeaprovacao} 
			...
			{\ABNTEXchapterfont\textsc{\large \imprimirtitulo}}
				...
				\begin{flushright}
					\begin{minipage}{6cm}	
				\centering
				Londrina, 06 de fevereiro de \imprimirdata.
					\end{minipage}
			\end{flushright}
...
\begin{resumo}
	...
	Este trabalho tem por objetivo verificar o impacto de choques de oferta e demanda sobre a inflação no contexto da economia brasileira e avaliar os efeitos destes sobre a eficácia do regime de metas de inflação (RMI) no Brasil. Será examinado o grau de eficiência das respostas do Banco Central e se as óticas atuais da condução da política monetária atendem de forma satisfatória os objetivos propostos pelo RMI. Por meio deste trabalho, busca-se atingir um melhor entendimento dos mecanismos que regem a dinâmica inflacionária no Brasil e como é moldada a resposta à inflação pela autoridade monetária brasileira. Para isso, será utilizado o modelo econométrico de vetores autorregressivos (VAR), que consegue capturar relações de longo prazo entre as variáveis escolhidas, que são a meta de inflação anual do Banco Central, a taxa de câmbio entre o Real e o Dólar Americano, o produto interno bruto (PIB) real do Brasil, o Índice Nacional de Preços ao Consumidor Amplo (IPCA) e o Índice de \textit{Commodities} - Brasil (IC-Br). Os resultados obtidos apontam que componentes de oferta são relevantes para a composição da inflação, mas podem ser negligenciados pela forma como o RMI é hoje estruturado.
...
\begin{siglas}
	...
	\item[IC-Br] Índice de \textit{Commodities} - Brasil
	...
	\item[SBIC] Critério de Informação Bayesiano
	\item[IPEA] Instituto de Pesquisa Econômica Aplicada
	\item[ERPT] Exchange Rate Pass-Through
\end{siglas}
\pagebreak
...
O controle da inflação é um dos grandes desafios enfrentados pelas autoridades econômicas em diversas nações ao redor do mundo. No Brasil, a adoção do Regime de Metas de Inflação (RMI) em 1999 representou uma mudança significativa na condução da política monetária, com o objetivo de estabilizar os níveis de preços e promover um ambiente econômico previsível \cite{fraga_2003_inflation}. A implementação do RMI é tida como um sucesso no geral, mas este está, como todo modelo, sujeito a ineficiências. Este trabalho tem como objetivo analisar o comportamento da dinâmica inflacionária no Brasil sob o RMI no período de 2002 a 2024, focando na efetividade do conjunto de políticas de contenção de inflação exercidas pelo Banco Central.

Para compreender o cenário a ser estudado, definimos o Regime de Metas de Inflação como um arranjo institucional em que o Banco Central se compromete a manter a inflação dentro de um intervalo pré estabelecido, utilizando instrumentos de política monetária, como as operações em \textit{open market}, para alcançar esse objetivo \cite{svensson_1997_inflation}. Este regime busca ancorar as expectativas inflacionárias dos agentes econômicos, contribuindo para a estabilidade macroeconômica \cite{mishkin_2000_inflation}, por meio de ajustes às taxas de juros da economia. Desde sua implementação, o RMI no Brasil tem se baseado em metas anuais de inflação definidas pelo Conselho Monetário Nacional (CMN), sendo o Banco Central responsável por ajustar a taxa do Sistema Especial de Liquidação e Custódia (SELIC) via instrumentos de política monetária para controlar a demanda agregada e manter a inflação dentro dos limites estipulados.

De acordo com o Banco Central do Brasil (2013), a taxa SELIC é obtida através do cálculo da taxa média ponderada e ajustada das operações de financiamento de um dia, que são garantidas por títulos públicos federais. Essas operações ocorrem no sistema financeiro ou em câmaras de compensação e liquidação de ativos, sob a forma de operações compromissadas, onde o vendedor se compromete a recomprar os títulos vendidos no dia útil seguinte. Abaixo, na equação \ref{eqn:selic}, é dada a fórmula utilizada para o cálculo da taxa:
...
A equação \ref{eqn:selic} baseia-se em três principais variáveis, sendo elas $L_j$, o fator diário correspondente à taxa da j-ésima operação, $V_j$, o valor financeiro correspondente à taxa da j-ésima operação e $n$, o número de operações que compõem a amostra. A taxa SELIC é principalmente ajustada pelas operações de mercado aberto (\textit{open market}), instrumento da política monetária onde o Banco Central realiza a compra ou venda de títulos da dívida pública aos principais bancos comerciais brasileiros, integrantes do que é denominado o nível 1.

Considerando isso, verifica-se que o ajuste da taxa SELIC reflete os movimentos do Banco Central no controle da inflação ao influenciar a oferta de moeda e o custo do crédito no mercado. A variação na taxa básica de juros, promovida pelas operações de mercado aberto, tem, portanto, efeitos diretos sobre os níveis de consumo e investimento na economia. A política monetária, ao atuar na direção de modificar a taxa SELIC, busca equilibrar o crescimento econômico com as pressões inflacionárias, que podem ter diferentes origens e dinâmicas.

Dessa forma, é importante verificar se a execução da política monetária como é definida atualmente gera um ambiente econômico excessivamente restritivo, onde o crédito se torna escasso e a reação às pressões inflacionárias geram taxas de juros muito elevadas, ainda mais quando se considera que o Brasil, em alguns momentos, possuiu uma das maiores taxas do mundo.

 A economia brasileira, como muitas outras, está sujeita a um descompasso entre o crescimento econômico real e a base monetária, resultando em inflação. Assim, detalharemos os diferentes tipos de inflação experienciados pelo país para analisar como o Banco Central tem respondido às pressões sobre os níveis de preços da economia. O processo inflacionário pode ter como fonte diversos fatores, sendo comum verificar a inflação de oferta, a inflação de demanda, a inflação inercial e a inflação estrutural. Cada um desses tipos de inflação apresenta características distintas que influenciam a eficácia do RMI, como será observado ao decorrer deste trabalho.

Como exemplo, a inflação de oferta decorre de choques nos custos de produção, como aumentos nos preços de matérias-primas advindas de importação ou salários \cite{blinder_2008_the}. Elevar a taxa de juros em resposta a esse tipo de inflação pode, porém, ser ineficaz, uma vez que não combate a causa raiz do processo inflacionário para esse caso em particular. Tratam-se de situações em que deve haver sobrecompensação da taxa de juros para evitar aumento no nível de preços. Isso pode reduzir o crescimento econômico demasiadamente e gerar impactos em setores alheios às causas do problema.

 Diante desse contexto, compreender a interação entre os tipos de inflação e o funcionamento do RMI é crucial para avaliar a eficácia das políticas monetárias adotadas pelo Banco Central do Brasil nas últimas décadas. Para isso, são utilizadas técnicas econométricas aplicadas a séries temporais dos índices de preços, em especial o IPCA (Índice de Preços ao Consumidor Amplo) e \textit{proxies} que o influenciam. Dessa forma, são utilizados outros indicadores macroeconômicos pertinentes, como o produto interno bruto (PIB), a taxa de câmbio entre o real brasilero e o dólar estadunidense e o indíce de comercialização de \textit{commodities} (IC-Br).
...
Desde o Cruzeiro, passando pelo Cruzado, até chegar ao Real, cada uma das moedas observadas na Tabela \ref{table:currencyhist} surgiu em resposta a crises inflacionárias e à perda de confiança na moeda nacional vigente na época. Essas mudanças foram frequentemente catalisadas por eventos externos, como crises internacionais e choques no preço do petróleo, mas também por fatores internos, como descontrole fiscal e políticas econômicas inconsistentes. Abaixo, detalhamos como a instabilidade econômica, que impacta diretamente a capacidade produtiva das empresas e o custo dos produtos e serviços, contribui para a inflação de oferta.

\section{Inflação de Oferta}

Um exemplo marcante de inflação de oferta, tanto no Brasil quanto em diversos outros países, ocorreu durante as crises do petróleo dos anos 1970. O aumento abrupto dos preços do petróleo, um insumo fundamental para diversas indústrias, resultou em custos mais altos de transporte e produção, impactando a economia brasileira. Outro exemplo mais recente foi observado durante a crise hídrica na região sudeste em 2014 e 2015, quando a escassez de água afetou a produção agrícola e a geração de energia hidrelétrica, levando ao aumento dos custos de alimentos e eletricidade \cite{crise_hidrica_lais}.
...
A indexação da economia, como já mencionada na seção \ref{seq:inflestr} de inflação estrutural, fazia com que o ajuste automático dos salários e contratos com base na inflação passada criasse um ciclo contínuo de reajustes. Essa indexação perpetuava a inflação ao longo do tempo, principalmente nos anos de 1980, quando esse comportamento começou a ser identificado a partir do desenvolvimento de teorias a respeito \cite{pereira2_98}.
...
\section{Perspectivas Recentes e Desafios (2023 em diante)}

Atualmente, o Brasil enfrenta uma série de desafios e oportunidades econômicas. O controle da inflação permanece sendo um ponto focal para o Banco Central. Sob a liderança de Roberto Campos Neto, a política monetária tem sido rigorosamente ajustada para garantir que a inflação fique dentro das metas estabelecidas, apesar das pressões inflacionárias globais e domésticas. Como é observado abaixo na Figura \ref{fig:ipcatgt}, isso fez com que o país tenha enfrentado taxas de inflação frequentemente maiores do que o planejado e isso se acentuou de forma severa após 2020:
...
Sob esse contexto, a alta taxa de juros necessária para manter a inflação sob controle tem tido um impacto negativo sobre o crescimento econômico e o emprego. Os custos de empréstimos elevados desincentivam investimentos e consumo, limitando o crescimento do PIB: ``Em relação ao setor produtivo, as altas taxas de juros foram apontadas como principal entrave ao crescimento econômico.'' \cite{omar_juros08}.

É interessante notar como crises econômicas	globais tiveram impacto sobre o ritmo do crescimento da inflação no Brasil. Com a crise da COVID-19, por exemplo, observou-se (Figura \ref{fig:ipcatgt}) a incapacidade das metas de inflação acompanharem a expansão monetária realizada nos períodos de gerenciamento da crise, evidenciando a dificuldade em manter a inflação sob controle. Diante disso, a próxima seção traz a escolha de modelos econométricos que propõem formas de analisar o comportamento da política monetária diante de tais cenários econômicos.
...
Como o objetivo deste estudo é analisar tanto a dinâmica inflacionária de oferta quanto de demanda, o VAR oferece a flexibilidade necessária para modelar essas influências e testar como a política monetária deve ser ajustada para alcançar os objetivos propostos pelo regime de metas de inflação. Em sua extensão, o modelo VAR oferece funções de resposta a impulsos (IRF) e decomposição de variância (VD), que serão utilizados para estudar o impacto de um choque nas variáveis do modelo sobre a inflação ao longo do tempo.

\section{Modelo}

Na equação \ref{eqn:varmodl} abaixo, é apresentado o modelo VAR de ordem $p$, onde um vetor de $n$ variáveis endógenas $y_t$ de dimensão $n \times 1$ é descrito por:

\begin{equation}
	\label{eqn:varmodl}
	y_t = \alpha + \Phi_1 y_{t-1} + \Phi_2 y_{t-2} + ... + \Phi_p y_{t-p} + u_t
\end{equation}

Onde:

\begin{itemize}
	\item $\alpha$ é um vetor $n \times 1$ de interceptos,
	\item Cada $\Phi_j$ é uma matriz $n \times n$ de coeficientes,
	\item $u_t$ é um vetor $n \times 1$ de termos de erro.
\end{itemize}

A estimação do modelo pode ser feita por Mínimos Quadrados Ordinários (MQO), aplicados equação a equação ou via métodos mais robustos e eficientes, dependendo das hipóteses sobre $u_t$ \cite{hamilton_1994_time_series_analysis}. A obtenção do comportamento da IRF é feita por meio da introdução de um choque sobre o modelo, sendo computado iterativamente pela decomposição autorregressiva do VAR \cite{lutkepohl_2005_new_introduction}.

Para uma construção eficaz do modelo, o primeiro passo é garantir que as variáveis utilizadas no modelo sejam estacionárias. A estacionariedade é fundamental para evitar resultados espúrios, ou seja, resultados que indicam uma relação de longo prazo inexistente entre as variáveis. De acordo com \citeonline{gujarati_porter_2011}, uma série temporal estacionária é aquela cujas propriedades estatísticas, como média, variância e autocorrelação, não mudam ao longo do tempo.

A verificação da estacionariedade é realizada por meio de testes como o Dickey-Fuller aumentado (ADF) e o Phillips-Perron (PP), que testam a presença de raiz unitária na série temporal. Caso as séries não sejam estacionárias, é possível realizar diferenciações para atingir a estacionariedade, como observado a seguir.
...
Para determinar corretamente como proceder com a diferenciação de uma série, surge a necessidade de verificar se cada série temporal possui raiz unitária e, portanto, não é estacionária. Nesse contexto, as seções subsequentes apresentam procedimentos para que isso seja feito.
...
A inclusão das defasagens \( \Delta y_{t-i} \) permite ao teste ADF lidar com séries temporais que apresentam autocorrelação serial nos erros. O número de defasagens \( p \) é selecionado com base em critérios de informação, como o Critério de Informação de Akaike (AIC) ou o Critério de Informação Bayesiano (SBIC), vistos posteriormente nas seções \ref{sec:aic} e \ref{sec:bic}, de modo a garantir que os erros \( \epsilon_t \) sejam não correlacionados e distribuídos de forma aproximadamente normal.
...
Uma vez identificada a presença ou não de raizes unitárias nas variaveis selecionadas, seja via ADF ou PP, é necessário determinar o numero de defasagens a serem adotas pelo modelo VAR. Abaixo, sao elencadas algumas formas de realizar uma selecao otima.

\subsection{Critério de Informação de Akaike (AIC)} \label{sec:aic}
...
Ao comparar diferentes modelos com defasagens variáveis, o modelo que apresenta o menor valor de AIC é escolhido como o modelo ótimo. No entanto, o AIC tende a ser mais permissivo em relação ao número de defasagens, selecionando frequentemente um número maior de \textit{lags} em comparação com outros critérios, como o Critério de Informação de Schwarz (SBIC) ou o Critério de Hannan-Quinn (HQIC), vistos a seguir.

\subsection{Critério de Informação de Schwarz (SBIC)} \label{sec:bic}

O Critério de Informação de Schwarz (SBIC), também conhecido como Critério de Informação Bayesiano, tem fórmula muito parecida com o AIC:

\begin{equation}
	\text{BIC} = -2 \ln(\hat{L}) + k \ln(n)
\end{equation}

A diferença chave entre os critérios é que o termo de penalização no BIC, \( k \ln(n) \), aumenta de forma logarítmica com o número de observações, tornando-o mais rigoroso conforme a amostra cresce. Isso implica que, para grandes amostras, o BIC tende a selecionar modelos com menos parâmetros em comparação ao AIC. No entanto, devido à sua penalização mais severa, o BIC pode, às vezes, subestimar o número de defasagens necessário, principalmente em séries onde há muita autocorrelação residual.
...
Além dos dados do Banco Central, o Instituto Brasileiro de Geografia e Estatística (IBGE) forneceu dados mensais da evolução do IPCA, a Fundação Getúlio Vargas (FGV) forneceu os dados de IGP-DI e o Instituto de Pesquisa Econômica Aplicada (IPEA) forneceu dados para compor as variáveis do PIB e da taxa de câmbio. 

O PIB foi inflacionado tomando como base o ano de 2024 utilizando o IGP-DI como inflator, isolando o efeito das flutuações de preços e garantindo que as comparações ao longo do período estudado reflitam mudanças reais na demanda e não na inflação. O IGP-DI foi escolhido como o índice inflator por ser um indicador abrangente que capta a variação de preços em diferentes setores da economia, incluindo o atacado, o consumidor e a construção civil, tornando-o uma medida robusta para corrigir o PIB nominal e convertê-lo em termos reais.

A taxa de câmbio utilizada no estudo expressa o valor da moeda brasileira em relação ao dólar comercial americano. A meta de inflação, definida pelo Conselho Monetário Nacional (CMN) e conduzida pelo BCB, serve como um guia para as expectativas inflacionárias e para a condução das taxas de juros via taxa SELIC. Ao incluir essa variável no modelo, é possível verificar se a inflação real (capturada pelo IPCA) converge para a meta estabelecida, o que permite avaliar o grau de sucesso e as possíveis deficiências da política monetária no controle da inflação ao longo do tempo. Além disso, essa variável permite observar como a política monetária responde a diferentes tipos de choques inflacionários ao longo do período analisado.

Levando o que foi apresentado em consideração, a seguir, são apresentadas a Tabela \ref{table:vars} e a Figura \ref{fig:ln_variaveis}, que sintetizam as variáveis, suas origens e as siglas que foram adotadas durante a execução da rotina econométrica desenvolvida no \textit{software} STATA, além de mostrá-las de forma visual:
...
\begin{table}[H]
	...
	\begin{tabular}{ | l || c | l | }
		\hline
		\textbf{Variável}                                                  & \textbf{Fonte}  & \textbf{Sigla} \\
		\hline
		\textbf{Índice de Preços ao Consumidor Amplo (IPCA)}               & IBGE/SNIPC      & ipca           \\
		\textbf{Taxa de Câmbio Comercial (PTAX)}                           & BCB             & cambio         \\
		\textbf{Taxa de Juros (\textit{Overnight}) SELIC}                  & BCB             & selic          \\
		\textbf{Produto Interno Bruto Real}                                & IBGE            & pib            \\
		\textbf{Produto Interno Bruto Inflacionado (IGP-DI)}               & Própria         & pibinfl        \\
		\textbf{Índice Geral de Preços - Disponibilidade Interna (IGP-DI)} & FGV             & igpdi          \\
		\textbf{Índice de \textit{Commodities} - Brasil (IC-Br)}           & BCB-Depec       & icbr           \\
		\hline
	\end{tabular}
	\vspace{1ex}
	\label{table:vars}
...
\begin{figure}[H]
	...
	\caption{Variáveis selecionadas em escala logarítimica ($\ln$) (2002-2024)}
	
	\includegraphics[width=\textwidth]{fig/ln_variaveis.png}\\
	\footnotesize{\textbf{Fonte}: o próprio autor, dados do IBGE e Banco Central}
	\label{fig:ln_variaveis}
\end{figure}

Com base nas variáveis selecionadas, definimos, a seguir, o modelo VAR que propõe a medição do impacto da oferta (\textit{IC-Br}) e da demanda (\textit{PIB}) da economia brasileira sobre a inflação (\textit{IPCA}):

% d.ln_ipca d.ln_cambio d.ln_selic d.ln_pibinfl d.ln_icbr
\begin{equation}
	\label{eqn:modvar}
	\Delta y_t
	=
	\begin{pmatrix}
		\Delta ln(\text{ipca}_t) \\
		\Delta ln(\text{cambio}_t) \\
		\Delta ln(\text{selic}_t) \\
		\Delta ln(\text{pibinfl}_t) \\
		\Delta ln(\text{icbr}_t) \\
	\end{pmatrix}
	=
	\alpha + \Phi_1 \Delta y_{t-1} + \Phi_2 \Delta y_{t-2} + ... + \Phi_4 \Delta y_{t-4} + u_t
\end{equation}
...
\begin{itemize}
	\item $y_t$ é o vetor $5 \times 1$ das variáveis endógenas no tempo $t$,
	\item $\alpha$ é o vetor $5 \times 1$ de interceptos,
	\item Cada $\Phi_j$ é uma matriz $5 \times 5$ de coeficientes,
	\item $u_t$ é o vetor $5 \times 1$ de termos de erro.
\end{itemize}

A seguir, procedemos os testes de raizes unitárias, verificamos a necessidade de diferenciação das séries temporais, selecionamos a defasagem ótima para o modelo, realizamos as estimações e geramos funções de impulso-resposta para verificar os choques sobre as variáveis escolhidas.

\section{Resultados e Discussão}

Os resultados dos testes DF-GLS e Phillips-Perron estão elencados na tabela \ref{table:adfpptests} abaixo. É evidente que as séries temporais não são estacionárias para nenhuma das variáveis a um nível de significância de $5\%$. Como observação, o prefixo ``$\ln\_$'' indica que a variável em questão foi transformada em logaritmo -- feito para que seja interpretada como elasticidade.

\begin{table}[H]
\centering
\caption{Resultados dos testes de raiz unitária DF-GLS e Phillips-Perron}
\label{table:adfpptests}
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{rcc|cc}
\hline
 & \multicolumn{2}{c|}{\textbf{Teste DF-GLS}} 
 & \multicolumn{2}{c}{\textbf{Teste Phillips-Perron}} \\
\hline
\textbf{Variável}    & Tau ($\tau$) & Limite 5\% & $Z(t)$ & Limite 5\% \\ 
\textit{ln\_ipca}    & -1.581       & -2.908     & -1.616 & -2.879     \\
\textit{ln\_cambio}  & -1.522       & -2.902     & -0.488 & -2.879     \\
\textit{ln\_selic}   & -3.529       & -2.885     & -2.083 & -2.879     \\
\textit{ln\_pibinfl} & -1.114       & -2.879     & -1.626 & -2.879     \\
\textit{ln\_icbr}    & -2.202       & -2.897     & -0.348 & -2.879     \\
\hline
\end{tabular}
\vspace{1ex}
\begin{minipage}{0.9\linewidth}
\footnotesize
\textbf{Fonte:} Elaborada pelo autor com software \textit{Stata}, utilizando os dados do estudo.\\
\textbf{Nota$^1$:} $^*$ denota rejeição da hipótese nula a 5\%.\\
\textbf{Nota$^2$:} Newey-West foi o critério de \textit{lags} para o teste de Phillips-Perron.\\
\textbf{Nota$^3$:} Ng-Perron foi o critério utilizado para o teste DF-GLS.
\end{minipage}
\end{table}

A partir dos resultados obtidos, fica nítida a necessadidade de se aplicar a diferenciação às variáveis. Após a aplicação das primeiras diferenças, observamos, através da tabela \ref{table:adfpptestsdiff}, que o critério de estacionariedade é satisfeito e as séries podem ser utilizadas como insumo para proceder o modelo VAR. Como observação, o prefixo ``$d.$'' demonstra que a primeira diferença foi utilizada para a variável em questão.

\begin{table}[H]
\centering
\caption{Resultados dos testes de raiz unitária DF-GLS e Phillips-Perron para variáveis em primeiras diferenças}
\label{table:adfpptestsdiff}
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{rcc|cc}
\hline
 & \multicolumn{2}{c|}{\textbf{Teste DF-GLS}} 
...
\hline
\textbf{Variável}    & Tau ($\tau$)  & Limite 5\% & $Z(t)$ & Limite 5\% \\ 
\textit{d.ln\_ipca}    & -6.353$^*$  & -3.480     & -7.681$^*$  & -2.879     \\
\textit{d.ln\_cambio}  & -9.456$^*$  & -2.908     & -17.089$^*$ & -2.879     \\
\textit{d.ln\_selic}   & -3.336$^*$  & -2.903     & -23.165$^*$ & -2.879     \\
\textit{d.ln\_pibinfl} & -5.560$^*$  & -2.879     & -20.398$^*$ & -2.879     \\
\textit{d.ln\_icbr}    & -10.228$^*$ & -2.908     & -12.367$^*$ & -2.879     \\
\hline
\end{tabular}
\vspace{1ex}
\begin{minipage}{0.9\linewidth}
...
Uma vez determinado que as variáveis atendem aos critérios de estacionariedade, procedemos os testes relacionados aos critérios de informação de Akaike, Hannan-Quinn e Schwarz. A tabela \ref{tab:defasagem} demonstra uma preferência, ao nível de significância de $5\%$, por quatro defasagens, apesar do critério SBIC demostrar apenas uma defasagen como ideal.

\begin{table}[H]
\centering
\caption{Resultado da seleção de defasagem ótima para os modelos}
\label{tab:defasagem}
\renewcommand{\arraystretch}{1.1}
\begin{tabular}{cccc}
\hline
\textbf{Defasagens} & AIC             & HQIC         & SBIC         \\
\hline
0                   & -20.8085        & -20.7817     & -20.7417     \\
1                   & -21.9777        & -21.8167     & -21.5768$^*$ \\
2                   & -22.1531        & -21.8580     & -21.4182     \\
3                   & -22.4956        & -22.0662     & -21.4265     \\
4$^*$               & -22.6587$^*$    & -22.0952$^*$ & -21.2556     \\
\hline
\end{tabular}
\vspace{1ex}
\begin{minipage}{0.9\linewidth}
\footnotesize
\textbf{Fonte:} Elaborada pelo autor com software \textit{Stata}, utilizando os dados do estudo.\\
\textbf{Nota:} $^*$ denota o número de defasagem escolhido pelo respectivo critério.
\end{minipage}
\end{table}


Enquanto o critério de informação de Schwarz indica o uso de uma defasagem, os outros critérios, AIC e HQIC apontam quatro defasagens como ideais para o modelo. Uma vez que o SBIC pune mais severamente subsequentes defasagens e que as séries temporais se extendem por um período de 20 anos, optamos por utilizar 4 \textit{lags}. Ao executar o modelo e obter sua IRF para as variáveis \textit{d.ln\_icbr} e \textit{d.ln\_pibinfl}, obtemos a figura \ref{fig:irf_ipca_2002_2024} abaixo:


\begin{figure}[H]
	
	\caption{IRF para choques de oferta e demanda sobre o IPCA (2002-2024)}
	
	\includegraphics[width=\textwidth]{fig/irf_ipca_2002_2024.png}\\
	\footnotesize{\textbf{Fonte}: o próprio autor, via \textit{software} STATA 17.}
	\label{fig:irf_ipca_2002_2024}
\end{figure}

Com base na figura \ref{fig:irf_ipca_2002_2024}, é possível observar que choques de demanda (PIB) são responsáveis apenas por parte do efeito observado sobre a inflação. Boa parte do comportamento do IPCA pode ser atribuído a choques de oferta (IC-Br). A implicação disso é que a política monetária brasileira atua sob eficiência reduzida, uma vez que seu principal mecanismo de ação é a contenção da demanda.

Adicionalmente, é possível explicitar outros fatores na composição da inflação. Notadamente, temos o efeito \textit{exchange rate pass-through} (ERPT), que, de acordo com \citeonline{cortapasso_estagflacao}, é o que descreve o grau de repasse da variação cambial para o nível de preços da economia. A figura \ref{fig:irf_cambio_2002_2024} demonstra que, além de choques de oferta e demanda, o efeito ERPT pode influenciar, ainde que em menor grau, a variação dos preços da economia.

\begin{figure}[H]
	
	\caption{IRF para choques no câmbio sobre o IPCA (2002-2024)}
	
	\includegraphics[width=\textwidth]{fig/irf_cambio_2002_2024.png}\\
	\footnotesize{\textbf{Fonte}: o próprio autor, via \textit{software} STATA 17.}
	\label{fig:irf_cambio_2002_2024}
\end{figure}

Em suma, o estudo demonstra que há outras vias pelas quais a política monetária pode atuar a fim de maximizar sua efetividade e promover a contenção da inflação de forma sustentável, evitando a estagnação econômica decorrente do encarecimento do crédito e do arrefecimento do ambiente econômico frequentemente observado nas implantações do regime de metas de inflação baseado puramente na ótica da demanda.

\chapter{Considerações Finais}

Os resultados apresentados ao longo deste trabalho evidenciam o caráter multifacetado da dinâmica inflacionária no Brasil. Embora a adoção do Regime de Metas de Inflação (RMI) em 1999 tenha sido bem-sucedida em consolidar as expectativas de estabilidade de preços, nossa análise reforçou que a inflação não é um fenômeno estritamente relacionado à demanda. Pelo contrário, choques de oferta (como variações nos custos de \textit{commodities}) também desempenham papel essencial na formação dos preços ao longo do período estudado (2002 a 2024). Isso sugere que o Banco Central, ao se apoiar sobretudo em políticas de elevação da taxa SELIC para conter a inflação, pode, em certos casos, desconsiderar a origem do choque inflacionário, o que torna o controle dos preços mais custoso para a economia.

A metodologia adotada, baseada em modelos de Vetores Autoregressivos (VAR), permitiu isolar e quantificar a resposta do IPCA a choques específicos de oferta e de demanda. A constatação de que fatores de oferta (como o índice de \textit{commodities} IC-Br) afetam de maneira não negligenciável a trajetória de preços reforça a relevância de políticas de estabilização que ultrapassem o escopo puramente monetário. Em cenários em que a pressão sobre a inflação advém predominantemente de choques de custo, reajustes intensos da taxa básica de juros podem, muitas vezes, elevar o custo de crédito e inibir o crescimento do PIB, sem resolver a raiz do problema inflacionário.

Da mesma forma, a avaliação das políticas econômicas e dos diferentes governos evidenciou a necessidade de coordenação entre política monetária e política fiscal. Periodicamente, o descompasso entre aumento de gastos públicos (ampliando a demanda agregada) e a tentativa de conter preços por meio de juros mais elevados acabou elevando o endividamento público e, consequentemente, a percepção de risco dos agentes econômicos. O equilíbrio entre disciplina fiscal, transparência nas ações do Banco Central e observância às metas inflacionárias mostrou-se crucial para a credibilidade da política monetária e para a confiança de investidores e consumidores.

Em termos de contribuições práticas, os resultados deste trabalho apontam para a importância de se considerar a natureza dos choques inflacionários antes de qualquer ação corretiva. Caso a origem seja a demanda, um incremento na taxa de juros pode ser justificado, mas, se, por outro lado, o choque advir de fatores de oferta, talvez sejam necessárias políticas setoriais ou estruturais, bem como estratégias de mitigação de volatilidade cambial, para reduzir a intensidade das pressões inflacionárias sem restringir excessivamente o investimento e o consumo. Ademais, a melhoria contínua na comunicação do Banco Central, alinhada a um debate público abrangente sobre metas e custos de combate à inflação, pode aprimorar a previsibilidade da política econômica e consolidar a ancoragem das expectativas inflacionárias.

Em síntese, o RMI continua relevante para a estabilidade de preços no Brasil, mas demanda um olhar atento para os diferentes motores da inflação. Se, por um lado, a forte tradição de indexação e a volatilidade econômica reforçam o papel de uma política monetária ativa, por outro, há a necessidade de complementá-la com reformas estruturais e maior coordenação fiscal. Dessa forma, a condução das políticas econômicas brasileiras poderá equilibrar a busca pela estabilidade de preços com a promoção de um ambiente macroeconômico menos suscetível a estagnações e mais propício ao crescimento de longo prazo.
